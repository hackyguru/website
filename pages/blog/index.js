...
const POSTS_PER_PAGE = 9; // Adjust this number as needed
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="flex-grow w-full grainy md:items-center md:justify-center bg-black/[0.96] antialiased bg-grid-white/[0.02] relative overflow-hidden">
        ...
        <div className='flex justify-between mt-32 px-10'>
          <div className=''>
            <h1 className="text-4xl font-mono mb-8 flex">{"/"}<Scramble text="Blog" /></h1>
          </div>
          <FilterBlogPosts
            tags={allTags}
        ...
        {loading ? (
          ...
          <div className="grid z-50 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-10">
            {currentPosts.map((post) => (
              <BlogCard className="z-20" key={post.id} post={post} activeTag={activeTag} />
            ))}
          </div>
        )}
      </div>
      <div className=" bottom-0 w-full bg-black/80 backdrop-blur-sm py-4">
        <div className="flex justify-center items-center space-x-4">
          <button
            ...
            className={currentPage === totalPages ? "border p-2 cursor-not-allowed opacity-50" : "border p-2 cursor-pointer"}
          >
            <ChevronLeft />
          </button>
